...
export class TextScanner extends EventDispatcher {
    ...
    async _search(textSource, searchTerms, searchKanji, inputInfo, showEmpty = false, disallowExpandStartOffset = false) {
        try {
            ...
            if (result !== null) {
                ...
                // Use the textSource content as sentence text so the VocabCurve
                // phrase entry can use it for local AI translation.
                const selectedText = textSource !== null ? textSource.text() : '';
                dictionaryEntries = [];
                sentence = {text: selectedText, offset: 0};
            }
    ...
    _onMousePointerMove(e) {
        ...
        // Suppress single-word scanning during Shift+LMB drag (multi-word selection in progress)
     ...
     */
    _onMousePointerUp(e) {
        // Shift+LMB release: scan the full text selection as a multi-word phrase
        if (e.shiftKey) {
            const selection = window.getSelection();
            if (selection !== null && !selection.isCollapsed && selection.toString().trim().length > 0) {
                const range = selection.getRangeAt(0);
                const textSource = TextSourceRange.createLazy(range);
                const modifiers = getActiveModifiersAndButtons(e);
                const modifierKeys = getActiveModifiers(e);
                const inputInfo = this._createInputInfo(null, 'mouse', 'click', false, modifiers, modifierKeys, {focus: true, restoreSelection: true});
                this._preventNextClick = true;
                void this._search(textSource, this._searchTerms, this._searchKanji, inputInfo, true, true);
            }
        }
    }
