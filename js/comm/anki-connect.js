...
export class AnkiConnect {
     ...
     * @returns {Promise<{[key: string]: {Front: string, Back: string}}>}
     */
    async getModelTemplates(modelName) {
        if (!this._enabled) { return {}; }
        await this._checkVersion();
        const result = await this._invoke('modelTemplates', {modelName});
        // Returns {"Card 1": {"Front": "...", "Back": "..."}}
        if (typeof result !== 'object' || result === null || Array.isArray(result)) {
            throw this._createUnexpectedResultError('object', result);
        }
        return /** @type {{[key: string]: {Front: string, Back: string}}} */ (result);
    }
     ...
     */
    async getModelStyling(modelName) {
        if (!this._enabled) { return {css: ""}; }
        await this._checkVersion();
        const result = await this._invoke('modelStyling', {modelName});
        if (typeof result !== 'object' || result === null || Array.isArray(result)) {
            throw this._createUnexpectedResultError('object', result);
        }
        return /** @type {{css: string}} */ (result);
    }

    /**
     * @param {string} modelName
     * @param {{[key: string]: {Front: string, Back: string}}} templates
     * @returns {Promise<null>}
     */
    async updateModelTemplates(modelName, templates) {
        if (!this._enabled) { return null; }
        await this._checkVersion();
        const result = await this._invoke('updateModelTemplates', {model: {name: modelName, templates: templates}});
        if (result !== null) {
            throw this._createUnexpectedResultError('null', result);
        }
        return null;
    ...
    async updateModelStyling(modelName, css) {
        if (!this._enabled) { return null; }
        await this._checkVersion();
        const result = await this._invoke('updateModelStyling', {model: {name: modelName, css: css}});
        if (result !== null) {
            throw this._createUnexpectedResultError('null', result);
        }
        return null;
