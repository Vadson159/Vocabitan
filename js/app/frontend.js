...
export class Frontend {
    ...
    /**
     * @param {{selectionText?: string}} [params]
     * @returns {void}
     */
    _onApiScanSelectedText(params) {
        const selectionText = params?.selectionText || '';
        void this._scanSelectedText(false, true, true, selectionText);
    }
     ...
     * @param {string} rawSelectionText raw text from window.getSelection().toString() (Google Translate approach)
     * @returns {Promise<boolean>}
     */
    async _scanSelectedText(allowEmptyRange, disallowExpandSelection, showEmpty = false, rawSelectionText = '') {
        ...
        // If we have raw selection text (from the selection icon, captured via
        // window.getSelection().toString() like Google Translate does), and it
        // differs from the DOM-based text, override the content.
        // This handles apps where words are in separate inline-block elements
        // without whitespace in the DOM, but with visual spacing via CSS.
        if (rawSelectionText && rawSelectionText !== source.text()) {
            source.overrideContent(rawSelectionText);
        }
