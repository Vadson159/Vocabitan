...
export class TextSourceRange {
    ...
    /**
     * Set whether the selection can be expanded.
     * @param {boolean} value
     */
    setDisallowExpandSelection(value) {
        this._disallowExpandSelection = value;
    }
    ...
    /**
     * Overrides the internal text content with a new string.
     * Used when the raw selection text (from window.getSelection().toString())
     * differs from the DOM-based textContent, e.g. when CSS generates visual
     * whitespace between inline-block elements that isn't present in the DOM.
     * This approach mirrors how Google Translate extracts text.
     ...
     */
    static create(range) {
        return new TextSourceRange(range, range.startOffset, TextSourceRange._getRangeText(range), null, null, null, null, false);
    }
    ...
    static createLazy(range) {
        return new TextSourceRange(range, range.startOffset, TextSourceRange._getRangeText(range), null, null, null, null, true);
    }
    ...
    static createFromImposter(range, imposterElement, imposterSourceElement) {
        ...
        return new TextSourceRange(range, range.startOffset, TextSourceRange._getRangeText(range), imposterElement, imposterSourceElement, cachedRects, cachedSourceRect, false);
    }

    /**
     * Extracts text content from a Range, filtering out content inside <sup> and <sub> elements.
     * @param {Range} range The source range.
     * @returns {string} The filtered text content.
     */
    static _getRangeText(range) {
        const fragment = range.cloneContents();
        const unwanted = fragment.querySelectorAll('sup, sub');
