...
export class Backend {
        ...
        try {
            ...
            if (options.general.enableContextMenuScanSelected) {
                chrome.contextMenus.create({
                    id: 'yomitan_lookup',
                    title: 'Lookup in Vocabitan',
                    contexts: ['selection'],
                }, () => this._checkLastError(chrome.runtime.lastError));
                chrome.contextMenus.onClicked.addListener((info, tab) => {
                    if (info.selectionText) {
                        const tabUrl = tab?.url;
                        const isNormalWebPage = (
                            typeof tabUrl === 'string' &&
                            (tabUrl.startsWith('http://') || tabUrl.startsWith('https://') || tabUrl.startsWith('file://'))
                        );
                        if (isNormalWebPage) {
                            // On normal web pages, try native in-page scan first.
                            // Also pass selectionText so the content script can use it as
                            // a fallback if window.getSelection() can't reach the text
                            // (e.g. inside another extension's injected shadow DOM).
                            this._sendMessageAllTabsIgnoreResponse({
                                action: 'frontendScanSelectedText',
                                params: {selectionText: info.selectionText},
                            });
                        }
                        // Always open the search popup with the selected text as a
                        // universal fallback. This reliably works for text from other
                        // extensions, restricted pages, and shadow DOM content.
                        void this._openSearchPopupWithText(info.selectionText);
                    }
                });
            } else {
    ...
    /**
     * Opens a temporary search popup window and fills it with the given text.
     * This window behaves like a tooltip: it auto-closes when losing focus.
     * @param {string} text The selected text to look up.
     * @returns {Promise<void>}
     */
    async _openSearchPopupWithText(text) {
        try {
            const url = chrome.runtime.getURL('/search.html') + '?context-menu-popup=true&query=' + encodeURIComponent(text);
            ...
            if (tabs && tabs.length > 0 && typeof tabs[0].id === 'number') {
                ...
                // Wait for the tab to initialize so we can hide the search UI
                await this._waitUntilTabFrameIsReady(tabId, 0, 2000);
                await this._sendMessageTabPromise(
                    tabId,
                    {action: 'searchDisplayControllerSetMode', params: {mode: 'context-menu-popup'}},
                    {frameId: 0}
                );
            }
            ...
            const onFocusChanged = (focusedWindowId) => {
                // WINDOW_ID_NONE can be fired occasionally, ignore it unless we're sure it's lost
